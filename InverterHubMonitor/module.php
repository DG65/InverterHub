    ...
    public function GetConfigurationForm()
    {
        ...
        if ($src > 0 && IPS_InstanceExists($src)) {
            ...
            if ($this->ReadPropertyInteger('IrradianceID') > 0) {
                $pvf = $this->PvfArea();
                if (!$pvf['installed']) {
                    $valueItems[] = ['type' => 'Label', 'caption' => 'ℹ Tipp: Mit dem Prognose-Modul „PV-Prognose" (Suite EnergiePrognose, DG65/Prognose) lässt sich der Einstrahlungssensor voll nutzen. Ist es installiert, berechnet der Monitor aus dessen Generatorparametern Erwartungswerte und stellt sie dem gemessenen Ertrag gegenüber (gestrichelt) — für Verschmutzungs- und Defekterkennung. Trägst du dort zusätzlich je Generator Modulanzahl sowie Modullänge/-breite ein (ab Version 0.20 / Build 41), kommen spez. Leistung und Performance-Ratio hinzu. Das Modul ist derzeit nicht installiert.'];
                } elseif ($pvf['area'] <= 0.0) {
                    $valueItems[] = ['type' => 'Label', 'caption' => 'ℹ Das Prognose-Modul „PV-Prognose" ist installiert, liefert aber noch keine Modulfläche. Diese gibt es erst ab Version 0.20 (Build 41) — im Stable-Kanal (0.19) ist sie noch nicht enthalten, dort hilft der Beta-Zweig des Repositories DG65/Prognose. Trage anschließend dort je PV-Generator die Modulanzahl sowie Modullänge und -breite (mm) ein — dann kann der Monitor die spez. Leistung / Performance-Ratio gegen die Einstrahlung auswerten. Die Erwartungswerte im Diagramm funktionieren unabhängig davon bereits mit der Stable-Version.'];
                } else {
